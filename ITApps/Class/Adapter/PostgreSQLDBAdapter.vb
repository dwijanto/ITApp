...
Public Class PostgreSQLDBAdapter
    ...
    Public Function isAdminParam(ByVal userid As String) As Boolean
        Dim sqlstr = String.Format("select * from it.user where userid = :userid")
        'Dim dbparams As New List(Of IDataParameter)
        'dbparams.Add(getParam("userid", userid.ToLower, DbType.String))

        Dim dbparam() As NpgsqlParameter = {getParam("userid", userid.ToLower, DbType.String)}
        'Dim dbparam() As NpgsqlParameter = Nothing
        Dim ra As Object = Nothing
        Dim message As String = String.Empty
        If ExecuteScalar(sqlstr, dbparam, ra, message) Then
    ...
    Function IsAdmin(ByVal p1 As String) As Boolean
        ...
        Try
            Using conn As New NpgsqlConnection(Connectionstring)
                conn.Open()
                Command = New NpgsqlCommand("it.sp_isadmin", conn)
